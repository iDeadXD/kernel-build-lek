...
then
	...
	KERNELSU_VERSION=$(git log --oneline KernelSU | head -n 1)
fi
...
then
  ...
  CHATID="${TELEGRAM_CHAT_ID}"
	...
	if [ -z "$TOKEN" ] || [ -z "$CHATID" ]; then
		msger -e "Telegram Token or Chat ID is missing! Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID variables."
	fi
fi
...
do_install_pkg(){
	sudo apt update -y
	sudo apt upgrade -y
	sudo apt install python3 cpio bc bison flex -y
}
 ...
 clone()
 {
	...
	if [ $COMPILER = "clang" ]
	then
		if [ ! -d "clang-llvm" ] || [ -z "$(ls -A clang-llvm 2>/dev/null)" ]; then
			msger -n "|| Clang not found or empty. Downloading... ||"
			mkdir -p clang-llvm
			wget "https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+archive/be2d8938fd43d57dea9aaccb18788cdb565ef727/clang-r596125.tar.gz" -O "clang.tar.gz"
			tar -xf clang.tar.gz -C clang-llvm
			rm clang.tar.gz
		else
			msger -n "|| Clang-LLVM folder found and populated. Skipping download. ||"
		fi
...
build_kernel()
{
	...
	then
	    ...
	    make -j"$PROCS" O=out \
		 "${MAKE[@]}" modules \
		 INSTALL_MOD_PATH="$KERNEL_DIR"/out/modules
	    make -j"$PROCS" O=out \
		...
		then
			...
			then
				...
				DTBO_PATH="vendor/qcom/fog-khaje-idp-nopmi-overlay.dtbo"
